...
from gmn_data_store import get_engine, get_session
from gmn_data_store.models import Meteor, Station, ParticipatingStation, \
    _FIELDS_IN_TRAJECTORY_SUMMARY_NOT_METEOR_TABLE, _Base, \
...
def insert_trajectory_summary(trajectory_summary_avro: Dict[str, Any], engine=None) -> None:
    """
    ...
    timestamp_fields = [item["name"] for item in schema["fields"] if item["type"] == ['null', {'type': 'long', 'logicalType': 'timestamp-micros'}]]

    meteor_fields = {}  # fields to add to Meteor table
    for field in Meteor.__table__.columns.keys():
        if field not in _FIELDS_IN_METEOR_TABLE_NOT_TRAJECTORY_SUMMARY and \
                field not in _FIELDS_IN_TRAJECTORY_SUMMARY_NOT_METEOR_TABLE:
            meteor_fields[field] = trajectory_summary_avro[field]

            # Special case for converting epoch microseconds to Datetime
            if field in timestamp_fields:
                meteor_fields[field] = datetime.datetime.fromtimestamp(meteor_fields[field] / 1e6)

    meteor_row = db_session.merge(Meteor(
        id=trajectory_summary_avro["unique_trajectory_identifier"],
        iau_shower_id=None if trajectory_summary_avro["iau_no"] == -1 else
        trajectory_summary_avro["iau_no"],
        **meteor_fields
    ))
    db_session.commit()
    ...
    for station_code in trajectory_summary_avro["participating_stations"]:
        ...
        existing_station = db_session.query(Station).filter_by(
            code=station_code).first()
        ...
        if not db_session.query(ParticipatingStation).filter_by(
                meteor_id=meteor_row.id, station_id=station_id).first():
            db_session.merge(ParticipatingStation(
                meteor_id=meteor_row.id,
