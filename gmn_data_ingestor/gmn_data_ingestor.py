...
# AVRO_SCHEMA = schema.parse(open(AVRO_SCHEMA_PATH).read())
# print(AVRO_SCHEMA)
...
def gmn_data_to_kafka(day_offset: int = 0):
    ...
    # trajectory_df = trajectory_df[['IAU (code)']]
    ...
    value_schema = avro.load(AVRO_SCHEMA_PATH)
    ...
    # producer = KafkaProducer(bootstrap_servers='kafka-broker:29092', value_serializer=serialize)
    #
    # for index, row in trajectory_df.iterrows():
    #     print("here1", AVRO_SCHEMA)
    #     print("here", row.to_dict())
    #     producer.send('extracted', row.to_dict())
